...
Drupal.behaviors.machineName = {
  ...
  attach: function (context, settings) {
    ...
    $.each(settings.machineName, function (source_id, options) {
      var $source = $(source_id, context);
      var $target = $(options.target, context);
      var $suffix = $(options.suffix, context);
      var $wrapper = $target.closest('.form-control');

      // Wrap the whole widget up. Can't do this in PHP because Drupal loves to
      // just throw random shit in and in reality the two form elements this
      // type outputs have no detectable relationship as far as rendering goes.
      // Only the action target element is classified as a machine name, not the
      // source.
      $source.parent('.form-control').wrap('<div class="form-component" data-component="machine-name"/>').parent().append($wrapper);
      ...
      var $preview = $('<output class="machine-name-value">' + options.field_prefix + Drupal.checkPlain(machine) + options.field_suffix + '</output>');
      $suffix.empty();
      if (options.label) {
        $suffix.append(' ').append('<label for="' + $target + '" class="machine-name-label">' + options.label + ':</label>');
      }
      ...
      var $link = $('<a class="button-small button-edit" href="#">' + Drupal.t('Edit') + '</a>')
        .click(function () {
          $wrapper.show();
          $target.focus();
          $suffix.hide();
